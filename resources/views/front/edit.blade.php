                    ...
                    <form action="{{ route('invoice.update' , $invoice->id) }}" method="post" class="form">
                        ...
                        <div class="table-responsive">
                            <table class="table" id="invoice_details">
                                ...
                                <tbody>
                                    @foreach($invoice->details as $item)                                    <tr class="cloning_row" id="{{ $loop->index }}">
                                        <td>
                                            @if($loop->index == 0)
                                                {{ '#' }}
                                            @else
                                        ...
                                        <td>
                                            <input type="text" name="product_name[{{ $loop->index }}]" id="product_name"
                                        ...
                                        <td>
                                            <select name="unit[{{ $loop->index }}]" id="unit" class="unit from-control">
                                                <option></option>
                                                <option value="{{ $item->unit == 'piece' ? 'selected' : '' }}">{{ __('frontend/frontend.piece') }}</option>
                                                <option value="{{ $item->unit == 'g' }} ? 'selected' : '' ">{{ __('frontend/frontend.gram') }}</option>
                                                <option value="{{ $item->unit == 'kg' }} ? 'selected' : '' ">{{ __('frontend/frontend.kilo_gram') }}</option>
                                            </select>
                                            @error('unit')
                                                <span class="help-block text-danger">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <td>
                                            <input type="number" name="quantity[{{ $item->index }}]" value="{{ old('quantity' , $item->quantity)}}"step="0.01" id="quantity"
                                        ...
                                        <td>
                                            <input type="number" name="unit_price[{{ $item->index }}]" value="{{ old('unit_price' , $item->unit_price) }}"step="0.01" id="unit_price"
                                        ...
                                        <td>
                                            <input type="number" step="0.01" name="row_sub_total[{{ $item->index }}]" value="{{ old('row_sub_total' , $item->row_sub_total )}}" id="row_sub_total"
                                ...
                                <tfoot>
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="number" step="0.01" name="sub_total" id="sub_total"
                                            value="{{ old('sub_total' , $invoice->sub_total) }}" class="sub_total form-control" readonly="readonly"></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3"></td>
                                        ...
                                        <td>
                                            <div class="input-group mb-3">
                                                <select name="discount_type" id="discount_type"
                                                    ...
                                                    <option value="percentage" {{ $invoice->discount_type == 'percentage' ? 'selected' : '' }}>{{ __('frontend/frontend.percentage') }}
                                                    </option>
                                                </select>
                                                <div class="input-group-append">
                                                    <input type="number" step="0.01" name="discount_value"
                                                        id="discount_value" class="discount_value form-control"
                                                        value="{{ old('discount_value' , $invoice->discount_value) }}" value="0.00">
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td><input type="number" step="0.01" name="vat_value" id="vat_value"
                                                class="vat_value form-control" readonly="readonly" value="{{ old('vat_value' , $invoice->vat_value) }}"></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3"></td>
                                        <td colspan="2">{{ __('frontend/frontend.shipping') }}</td>
                                        <td><input type="number" step="0.01" name="shipping" id="shipping"
                                                vlaue="{{ old('shipping' , $invoice->shipping) }}" class="shipping form-control" ></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3"></td>
                                        <td colspan="2">{{ __('frontend/frontend.total_due') }}</td>
                                        <td><input type="number" step="0.01" name="total_due" id="total_due"
                                                vlaue="{{ old('total_due' , $invoice->total_due) }}" class="total_due form-control"  readonly="readonly"></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
